📚 Grade 12 Study Web App

A creative and interactive study platform designed to empower Grade 12 learners with the tools they need to study smarter, stay motivated, and bag distinctions. 
Built with love and ambition to make academic excellence fun, trackable, and community-driven.

💻 Created by Nontando Myoli

## 🌐 Live Demo
Access the web app here 👉 

🚀 Features
✅ Timetable Generator
Create your own weekly study schedule by selecting Grade 12 subjects and time slots.

🧠 Quiz Mode
Practice MCQ-style quizzes to prepare for tests and final exams. More subjects will be added over time!

📄 Past Papers Section
Quick access to previous exam papers for real-time practice.

🎯 Bag Distinctions Page
Features:

🎓 Motivational study tips

✅ Revision task checklist

📈 Progress bar tracker

🎯 Subject goals section

🤖 Interactive Chatbot
A helpful, nameless JavaScript-powered chatbot that assists with Life Sciences queries and is always ready to chat while you revise!

💬 Student Community Groups (Upcoming)
Join virtual discussion spaces for each subject to ask questions, collaborate, and study together.

🛠️ Tech Stack
HTML5

CSS3

JavaScript (Vanilla)

Future Improvements
Here’s what’s coming soon:

📚 Additional Subjects Integration
Other Grade 12 subjects like Business Studies, Geography, Economics, Tourism, Visual Arts, etc. will be included gradually.

🧑🏽‍🤝‍🧑🏾 Student Virtual Study Groups
Chatrooms or forums for each subject to encourage peer learning.

🏆 Gamification with Badges
Earn badges for completing topics, consistency, or high quiz scores!

📊 Performance Analyzer
Track accuracy, speed, and progress across topics to guide smarter revision.

📤 Upload & Share Notes
Learners can upload and download summarized notes and resources.

📚 Resource Repository
A central bank of curated links, guides, video lessons, and printable worksheets.

🤖 AI Support Bots for Subjects
Starting with Life Sciences, with more bots for other subjects coming soon.

📱 Mobile Application (Android/iOS)
A responsive mobile app version for offline learning and full functionality on the go.

🔔 Notifications & Daily Study Reminders

🌙 Dark Mode (with warm yellow & orange tones)
A relaxing interface for late-night sessions.

🔐 User Login + Save Progress

📅 Calendar & Planner Integration

📸 Screenshots (Optional)
Insert screenshots of each page if you'd like to include them later.

This project is part of a mission to make studying beautiful, motivating, and impactful. Created to inspire learners to reach their full potential in Grade 12 and beyond.


